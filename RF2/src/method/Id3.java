...
public class Id3 {
	...
	public Double getGain(LinkedList<String[]> lines, int index) {
		...
		for(int i=0;i<lowvalue.length;i++)
			Sv+=lowvalue[i];
		Double entropySv = TheMath.getEntropy(Sv,lowvalue);
		Point lp = new Point();
		lp.setSv(Sv);//该属性某个取值存在的数据集数目
		...
		for(int i=0;i<highvalue.length;i++)
			Sv+=highvalue[i];
		entropySv = TheMath.getEntropy(Sv,highvalue);
		Point hp = new Point();
		hp.setSv(Sv);//该属性某个取值存在的数据集数目
	...
	public void createDTree() {
		root = new TreeNode();
		Maxgain maxgain = getMaxGain(subdata);
		if (maxgain == null) {
			System.out.println("没有数据集，请检查!");
		}
		if (maxgain.getMaxgain() == -1.0)// 说明训练数据集属于同一类
		{
		...
		else if (maxgain.getMaxgain() == -2.0) {// 属性集合为空
